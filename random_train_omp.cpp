// openmp_training.cpp : Defines the entry point for the console application.
//
...
#pragma omp parallel for
	...
	int i;
#pragma omp parallel for
	for (i = 0; i<subprobNo*subprobNo; i++)
	{
...
#pragma omp parallel for
	...
	*/
	...
	FPR = 1.*FP / (FP + TN);
	cout << "---------------------------------------------------\n"
		<< "train and minmax time: " << (float)total / CLOCKS_PER_SEC << 's' << endl
...
{
	vector<int> res = genRandomIndex(10);
	for (auto iter : res)
		cout << iter << ' ';
	cout << endl;
}
...
int main(int argc, char** argv)
{
	...
	clock_t end = clock();
	cout <<"total running time: "<< (end - start)*1. / CLOCKS_PER_SEC << endl;
